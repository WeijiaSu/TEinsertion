...
# Add reference length and select the reads: 1. have full-length TE; 2. have at least 100 bp flanking sequence on both side.
def AddTElength(file):
	...
	f=f.loc[(f["ReadStart"]>=100) | (f["ReadEnd"]<=f["ReadLen"]-100)]
	f=f.loc[((f["RefStart"]<=10) & (f["RefEnd"]>=100)) & ((f["RefEnd"]>=f["RefLen"]-10) & (f["RefStart"]<=f["RefLen"]-100))]
	print(f[)])
	f.to_csv(file+"_TE.tsv",index=None,sep="\t")
